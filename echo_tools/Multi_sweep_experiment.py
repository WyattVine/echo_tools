...
class Multi_sweep_experiment():
    ...
    def read_data(self):
        if self.data_name_conventions.values()[0]:
    ...
    def overlay_traces(self,save_name=None,num_cols=3,order=self.sweep_labels,legend_loc=0,**kwargs):
    ...
    def compare_2D_plots(self,save_name=None,order=self.sweep_labels,**kwargs):
    ...
    def integrate_echos(self,with_discriminators=False,std_multiplier=1,**kwargs):
        ...
        for i in self.sweeps:
            i.integrate_echos(with_discriminators=with_discriminators,std_multiplier=std_multiplier)
